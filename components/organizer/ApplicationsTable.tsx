...
function pickFieldKey(fields: Field[], labelMatchers: RegExp[], keyFallbacks: string[]) {
  const byLabel = fields.find((f) => labelMatchers.some((re) => re.test(f.label)));
...
export default function ApplicationsTable({
  ...
  async function load() {
    ...
    const res = await fetch(`/api/forms/${formId}/applications`, { cache: "no-store" });
  ...
  const locationKey = useMemo(
    () => pickFieldKey(fields, [/location/i, /city/i, /state/i, /where.*based/i], ["location", "Location"]),
    [fields]
  );
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white shadow-[0_1px_0_rgba(0,0,0,0.02)]">
        <div className="h-[calc(100vh-260px)] overflow-auto">
          ...
          <table className="w-full min-w-[1500px] table-fixed text-[14px]">
            <colgroup><col style={{ width: "56px" }} /><col style={{ width: "210px" }} /><col style={{ width: "220px" }} /><col style={{ width: "180px" }} /><col style={{ width: "140px" }} /><col style={{ width: "140px" }} /><col style={{ width: "170px" }} /><col style={{ width: "200px" }} /><col style={{ width: "260px" }} /><col style={{ width: "200px" }} /><col style={{ width: "140px" }} /><col style={{ width: "44px" }} /><col style={{ width: "44px" }} /></colgroup>

            <thead className="sticky top-0 z-20">
              <tr className="bg-zinc-50 text-zinc-600 border-b border-zinc-200">
                <th className="sticky left-0 z-30 bg-zinc-50 px-4 py-4 font-medium text-left">#</th>
                <th className="px-4 py-4 font-medium text-left">Time Submitted</th>
            ...
            <tbody className="text-zinc-700">
              ...
              {filteredApps.map((a, idx) => {
                ...
                const appEmail =
                  (emailKey ? (a.answers?.[emailKey] as string | undefined) : undefined) ??
                  a.applicant.email ??
                  "—";
                ...
                const appLocation =
                  ...
                  (locationKey ? (a.answers?.[locationKey] as string | undefined) : undefined) ??
                  "—";
                ...
                return (
                  <tr key={a.id} className="border-b border-zinc-100 hover:bg-zinc-50/60">
                    ...
                    <td className="px-4 py-5 text-zinc-400 truncate">{notes}</td>
                    ...
                    <td className="px-4 py-5">
                      <div className="inline-flex items-center rounded-full bg-zinc-100 px-3 py-1.5">
                        ...
                        <span className="ml-[-18px] pointer-events-none text-zinc-400">▾</span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-5 truncate">{String(appLocation)}</td>
        ...
        <div className="flex items-center justify-between border-t border-zinc-100 px-4 py-3 text-sm text-zinc-500">
          <div>
            Showing <span className="text-zinc-700">{filteredApps.length.toLocaleString()}</span> of{" "}
            <span className="text-zinc-700">{apps.length.toLocaleString()}</span>
          </div>
          <button
            type="button"
